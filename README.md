...
<p align="center">
  <a href="#установка"><img src="https://img.shields.io/badge/версия-3.33-8ECA43?style=flat-square" alt="версия"></a>
...
| Объём за сутки | 25 ГБ | восемь часовых порогов |
...
| Наказание | 1 Мбит/с на 60 мин | то же |
...
Сутки у домашних — восемь таких часов. Держать половину канала треть суток это
уже не «посмотрел кино».
...
### Быстрая нода: половина канала за час

Четвёртый пресет отличается от остальных тем, что **не задаёт часовой порог
числом, а вычисляет его от лимита скорости**.

Причина в том, что гигабайты за час сами по себе ничего не значат:
...
срабатывает от одного фильма. Поэтому пресет берёт **половину канала за
час**:
...
| лимит | порог |
|---|---|
| 10 Мбит/с | 2.2 ГБ/час |
| 50 Мбит/с | 11.2 ГБ/час |
| 100 Мбит/с | 22.5 ГБ/час |

Смысл один: «держал больше половины своей полосы целый час». Видео берёт от
этого малую долю — 4K это около 7 ГБ/час, — а непрерывная закачка забирает
всё и попадается.
...
чего — пресет честно говорит об этом и предлагает фиксированные 20 ГБ.

Остальные значения: суточный порог 100 ГБ как страховка для медленных, но
упорных, и штраф 1 Мбит/с на час. Час, а не четыре: срабатывание уже строгое,
а тот, кто продолжит, попадётся снова.
