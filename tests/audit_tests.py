...
# Страна в подписи: на живой ноде нашлись турецкий провайдер и хостинг с
# Сейшел, обе корзины выше порога. Их падение к ТСПУ отношения не имеет, и
# видно это должно быть в момент чтения тревоги.
check("подпись сети содержит страну", "ZZ" in S.asn_name(_tab, 64496),
      S.asn_name(_tab, 64496))
check("порядок в подписи: номер, страна, название",
      S.asn_name(_tab, 64496) == "AS64496 ZZ Оператор один", S.asn_name(_tab, 64496))
...
_N1, _N2 = "AS64496 ZZ Оператор один", "AS64497 ZZ Оператор два"
...
check("сеть ниже порога не судим",
      "AS64498 ZZ Оператор три" not in _told, _told)
...
# Наплыв на ноде за CDN: область ушла под белые списки, люди перетекли сюда.
_T6 = 9_000_000.0
_fill(40, 5, 30, _T6)
...
check("сообщение говорит про наплыв, а не про падение",
      len(_sent) == 1 and "наплыв" in _sent[0].lower(), _sent)

# Порог наплыва считается по результату, а не по норме: с трёх до семи —
# удвоение, но доказывать нечего.
_T7 = 9_500_000.0
_fill(40, 3, 30, _T7)
...
# настоящая тревога оказалась ложной. Проверка по разбросу это снимает.
_DEHOST = [14, 20, 9, 15, 13, 9, 6, 20, 22, 22]
check("шумный хостинг не объявляется упавшим",
      not S._fell(6, S._median(_DEHOST), S._mad(_DEHOST)),
      "медиана %s, MAD %.1f" % (S._median(_DEHOST), S._mad(_DEHOST)))

# Билайн на той же ноде за тот же час: 52..67. Настоящий оператор стабилен,
# и падение вдвое обязано пройти проверку, иначе она глушит сигнал.
_BEE = [61, 58, 63, 67, 64, 63, 62, 62, 59, 54]
check("падение стабильного оператора вдвое замечается",
      S._fell(31, S._median(_BEE), S._mad(_BEE)),
      "медиана %s, MAD %.1f" % (S._median(_BEE), S._mad(_BEE)))
check("обычные колебания оператора не тревожат",
      not S._fell(54, S._median(_BEE), S._mad(_BEE)))
check("ровная история судится по одному отношению",
      S._fell(4, 20, 0.0) and not S._fell(11, 20, 0.0))
# Отношение прошло, разброс — нет: падения с 20 до 9 мало для сети, которая
# сама по себе гуляет на четыре.
check("разброс может отменить падение, прошедшее по отношению",
      not S._fell(9, 20, 4.0))
check("но при малом разбросе то же падение засчитывается",
      S._fell(9, 20, 1.0))
...
# ── Наплыв ─────────────────────────────────────────────────────────────
# Когда в области включают белые списки, люди не исчезают, а перетекают на
# ноду за CDN. Там их сеть растёт, и увидеть это можно раньше, чем на обычных
# нодах просядет достаточно для порога.
check("событие о наплыве объявлено", "censor_rise" in S.EVENT_TYPES)
check("порог наплыва — не меньше удвоения", S.CENSOR_RISE >= 2)

# Тот же ряд МТС с ноды: он сам по себе сходил с 69 до 110, то есть вырос на
# 59%. Полуторакратный порог объявил бы это наплывом, удвоение — нет.
_MTS = [66, 67, 96, 110, 84, 62, 67, 61, 69, 71]
check("естественный скачок оператора наплывом не считается",
      not S._rose(110, S._median(_MTS), S._mad(_MTS)),
      "медиана %s, MAD %.1f" % (S._median(_MTS), S._mad(_MTS)))
check("настоящее удвоение считается",
      S._rose(150, S._median(_MTS), S._mad(_MTS)))
check("ровная история судится по одному отношению и здесь",
      S._rose(40, 20, 0.0) and not S._rose(39, 20, 0.0))

# Живой случай. Переход на хранение по номерам сетей оставил в истории смесь:
# старые свёрнутые подписи операторов и новые по номерам. У новых ключей нормы
# не было вовсе, медиана вышла нулевой, и три ноды разом объявили наплыв всему,
# что видели — «сейчас 112, обычно 0». Сеть, которой не с чем сравнить, не
# может считаться пришедшей.
check("нулевая норма наплывом не считается", not S._rose(112, 0, 0.0))
check("норма в один адрес уже считается", S._rose(40, 1, 0.0))

_mixed = ([{"МТС": 50}] * 6 + [{"AS8359 RU MTS": 50}] * 4)
_mkey = "AS8359 RU MTS"
_mvals = [int(h.get(_mkey, 0)) for h in _mixed]
check("на смешанной истории новый ключ наплыва не даёт",
      not S._rose(56, S._median(_mvals), S._mad(_mvals)),
      "медиана %s" % S._median(_mvals))

# Разбор наплыва по сетям — то же, что для падения, но в другую сторону.
_rbase = [{"AS64496 RU MTS": 40, "AS64497 RU MTS-PENZA-AS": 5}] * 10
_rcur = {"AS64496 RU MTS": 42, "AS64497 RU MTS-PENZA-AS": 45}
_rblame = S.censor_blame(S._ASN_TABLE, _rbase, _rcur, "МТС", rising=True)
check("наплыв разложен по сетям, виновник первым",
      _rblame and _rblame[0][1] == "AS64497 RU MTS-PENZA-AS", _rblame)
