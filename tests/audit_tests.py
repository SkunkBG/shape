...
# Байты и пакеты — одно поле из двух чисел. Два поля можно получить
# наполовину: запись от прошлой версии, где было только одно из них.
check("испорченное поле не роняет строку",
      "·" not in S.penalty_figures(dict(D, upkt="мусор")).split("(")[1])
