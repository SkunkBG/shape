...
# секунду. Останавливает block — минимальная скорость на все его адреса, час
# подписка не работает ни у кого из покупателей. Соединения при этом НЕ рвутся:
# лимит действует на открытые сразу, а обрыв стёр бы сессии из панели, и
# владелец, пришедший по уведомлению, увидел бы пустую карточку.
check "раздача перекрывает доступ, а не только рвёт" \
      '[[ $(grep -c -- "--action-set block >/dev/null" "$SRC/menu.sh") -eq 2 ]]'
check "перекрытие не тянет за собой обрыв" \
      '! grep -q "\"drop\" in actions or \"block\" in actions" "$SRC/shaperctl.py"'
