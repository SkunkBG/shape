...
# Считаем только пресеты: у ручной настройки значение подставляется
# переменной, и в этот счёт она попадать не должна.
check "каждый пресет задаёт отношение ровно один раз" \
      '[[ $(grep -c -- "--upload-ratio [0-9]" "$SRC/menu.sh") -eq 5 ]]'
